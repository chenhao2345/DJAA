...
from djaa.evaluators_compatible import Evaluator_compatible, extract_features
...
def evaluate_all(args, datasets, evaluator_1_ema, gallery_buffer):
    rank1 = []
    rank5 = []
    rank10 = []
    mAP = []
    for dataset in datasets:
        ...
        (cmc, mAP_1), gallery_buffer = evaluator_1_ema.evaluate(test_loader, dataset.query, dataset.gallery, cmc_flag=True,
                                                              gallery_buffer=gallery_buffer)
        rank1.append(cmc[0])
        rank5.append(cmc[4])
        rank10.append(cmc[9])
        mAP.append(mAP_1)
...
def main_worker(args):
    ...
    initial_weights = load_checkpoint('examples/logs/lsktc_order2/step1.pth.tar')
    copy_state_dict(initial_weights['state_dict'], model_1)

    # Evaluator
    evaluator_1 = Evaluator_compatible(model_1)
    gallery_buffer1 = evaluate_all(args, datasets, evaluator_1, gallery_buffer=None)

    initial_weights = load_checkpoint('examples/logs/lsktc_order2/step2.pth.tar')
    copy_state_dict(initial_weights['state_dict'], model_1)

    # Evaluator
    evaluator_1 = Evaluator_compatible(model_1)
    gallery_buffer2 = evaluate_all(args, datasets, evaluator_1, gallery_buffer=gallery_buffer1)

    initial_weights = load_checkpoint('examples/logs/lsktc_order2/step3.pth.tar')
    copy_state_dict(initial_weights['state_dict'], model_1)

    # Evaluator
    evaluator_1 = Evaluator_compatible(model_1)
    gallery_buffer3 = evaluate_all(args, datasets, evaluator_1, gallery_buffer=gallery_buffer1)
