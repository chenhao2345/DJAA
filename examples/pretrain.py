...
from djaa.trainers import ImageTrainer_memory, PreTrainer
...
from djaa.utils.data.sampler import MoreCameraSampler, RandomIdentitySampler, RandomMultipleGallerySampler
...
def get_pretrain_loader(dataset, height, width, batch_size, workers,
    ...
    if rmgs_flag:
        ...
        sampler = RandomMultipleGallerySampler(train_set, num_instances, video=True)
...
def create_model(args, num_pids):
    model_1 = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=num_pids, pooling_type=args.pooling_type)

    model_1_ema = models.create(args.arch, num_features=args.features, dropout=args.dropout, num_classes=0, pooling_type=args.pooling_type)
...
def lifelong_supervised_trainer(args, dataset_target, model_1, model_1_ema, optimizer, lr_scheduler, evaluator_1_ema,
    ...
    #     dict_f1, _ = extract_features(model_1_old, cluster_loader, print_freq=50)
    #     cf_model_old = torch.stack(list(dict_f1.values()))
    ...
    for epoch in range(args.epochs):
        ...
        for id in range(num_ids):
            ...
            # if (centers_old is not None) and (dictionary_old is not None):
            # centers_model_old.append(torch.mean(cf_model_old[labels == id], dim=0))
        ...
        num_img_old = 0
        if (centers_old is not None) and (dictionary_old is not None):
            # centers_model_old = torch.stack(centers_model_old, dim=0)
            # centers_model_old = torch.cat([centers_model_old, centers_old], dim=0)
        ...
        train_loader_target = get_pretrain_loader(dataset_target, args.height, args.width,
        ...
        if (centers_old is not None) and (dictionary_old is not None):
            ...
            train_loader_target_newold = get_train_loader(dataset_target, args.height, args.width,
                                                       args.batch_size, args.workers, args.mem_samples, args.iters,
                                                       trainset=pseudo_labeled_dataset_newold, mutual=True, index=True)
        ...
        #                               memory=None, scale_kl=args.scale_kl, model_1_old=model_1_old, lambda_kl=args.lambda_kl)
        #
        ...
        #               train_loader_target_old=train_loader_target_old, train_loader_target_newold=train_loader_target_newold)
        #
        # lr_scheduler.step()

        # Pretrain Trainer
        trainer = PreTrainer(model_1, num_ids, margin=0.3)

        trainer.train(epoch, train_loader_target, optimizer, train_iters=len(train_loader_target), print_freq=10)

        lr_scheduler.step()

        if (epoch + 1) % args.eval_step == 0:
            cmc, mAP_1 = evaluator_1.evaluate(test_loader_target, dataset_target.query, dataset_target.gallery, cmc_flag=True)
    ...
    if len(pseudo_labeled_dictionary) > mem_num:
        ...
        centers = centers[idx]
        # centers_hard = centers_hard[idx]
        # centers_easy = centers_easy[idx]

        # for key, values in new_dictionary.items():
        #     for i in values:
        #         print(key, " : ", i)

        market_count, cuhk_count, msmt_count, personx_count = 0, 0, 0, 0
        for j, per_id in enumerate(new_dictionary.values()):
            for (fname, pid, cid, _) in per_id:
                if 'msmt17' in fname:
                    ...
                    cuhk_count += 1
                elif 'personx' in fname:
                    personx_count += 1
        print('Memory Update')
        print(f'personx_count:{personx_count}, market_count:{market_count}, cuhk_count:{cuhk_count}, msmt_count:{msmt_count}')
        return model_1, model_1_ema, centers, new_dictionary
    # elif len(pseudo_labeled_dictionary) < mem_num:
...
def lifelong_unsupervised_trainer(args, dataset_target, model_1, model_1_ema, optimizer, lr_scheduler, evaluator_1_ema,
    ...
    for epoch in range(args.epochs):
        ...
        # print(sys.getsizeof(centers.storage()))
...
def main_worker(args):
    ...
    # Create data loaders
    dataset_personx = get_data('personx', args.data_dir)
    dataset_market = get_data('market1501', args.data_dir, split=2)
    # dataset_duke = get_data('dukemtmc-reid', args.data_dir)
    dataset_cuhksysu = get_data('cuhk-sysu', args.data_dir, split=1)
    dataset_msmt = get_data('msmt17', args.data_dir, split=1)

    dataset_market2 = get_data('market1501', args.data_dir, split=1)
    # dataset_duke = get_data('dukemtmc-reid', args.data_dir)
    ...
    datasets = [dataset_personx, dataset_market, dataset_cuhksysu, dataset_msmt, dataset_market2, dataset_cuhksysu2, dataset_msmt2]
    # datasets_unseen = [dataset_cuhk03, dataset_ilids, dataset_viper, dataset_3dpes]
    datasets_eva = [dataset_personx, dataset_market, dataset_cuhksysu]
    ...
    # Create model
    model_1, model_1_ema = create_model(args, num_pids=410)
    ...
    print('Pre-training on dataset # {}.'.format(0))
    model_1, model_1_ema, centers, pseudo_labeled_dictionary = lifelong_supervised_trainer(args, datasets[0], model_1, model_1_ema, optimizer, lr_scheduler, evaluator_1_ema)
    save_dir = osp.join(args.logs_dir, 'split_DJAA_pretrain')
    os.makedirs(save_dir,exist_ok=True)
    save_checkpoint({'state_dict': model_1_ema.state_dict()}, False, fpath=osp.join(save_dir, 'step{}.pth.tar'.format(0)))
    model_1_old = copy.deepcopy(model_1_ema)
    lr_scheduler = WarmupMultiStepLR(optimizer, args.milestones, gamma=1, warmup_factor=0.1,
                                     warmup_iters=args.warmup_step)

    evaluate_all(args, datasets_eva, evaluator_1)
    evaluate_all(args, datasets_unseen_mmp, evaluator_1)
    ...
    #     model_1, model_1_ema, centers, pseudo_labeled_dictionary = lifelong_unsupervised_trainer(args,
    #                                                                                              dataset,
    #                                                                                              model_1,
    #                                                                                              model_1_ema,
    #                                                                                              optimizer,
    #                                                                                              lr_scheduler,
    #                                                                                              evaluator_1_ema,
    #                                                                                              model_1_old=model_1_old,
    #                                                                                              centers_old=centers,
    #                                                                                              dictionary_old=pseudo_labeled_dictionary)
    ...
    #     save_checkpoint({'state_dict': model_1_ema.state_dict()}, False, fpath=osp.join(save_dir, 'step{}.pth.tar'.format(i+1)))
    #
    #     model_1_old = copy.deepcopy(model_1_ema)
    #     lr_scheduler = WarmupMultiStepLR(optimizer, args.milestones, gamma=1, warmup_factor=0.1,
    #                                      warmup_iters=args.warmup_step)
